...
- Build the docker prod image `docker-compose build prod` (dev: `docker-compose build dev`)
- Create and run a docker container `docker-compose up -d prod` (dev: `docker-compose up -d dev`)
...
- `docker-compose stop prod` (dev: `docker-compose stop dev`) - stops the certain container.
